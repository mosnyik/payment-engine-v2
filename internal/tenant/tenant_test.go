...
func TestApproveKYB_ThenIssueAndAuthenticate(t *testing.T) {
	...
	gotSecret, gotTenantID, _, ok, err := s.LookupHMACSecret(ctx, apiKey)
	if err != nil {
...
func TestSuspend_RevokesAuthenticationImmediately(t *testing.T) {
	...
	_, _, _, ok, err := s.LookupHMACSecret(ctx, apiKey)
	if err != nil || !ok {
...
func TestLookupHMACSecret_UnknownKey(t *testing.T) {
	...
	_, _, _, ok, err := s.LookupHMACSecret(ctx, "pk_does_not_exist")
	if err != nil {
